...
public class ControllerMethod extends AbstractHandler{
	...
	private void doPorcess(TextSelection selection, IDocument document, ITextEditor ite) {
		...
		String reqVOParam = selectedStr+"ReqVO";
		String reqVOType = StringUtils.capitalize(reqVOParam);
		...
		returnTypeStr = "ResponseWrapper<"+returnTypeStr+">";
		//方法
		String replaceFunc = 
				"/**\n"
				+ "\t * \n"
				...
				+ "\tpublic "+returnTypeStr+" "+selectedStr+"(@RequestBody @Validated "+reqVOType+" "+reqVOParam+") {\n"
				+ "\t\treturn new ResponseWrapper(true,ApiErrorResponse.CODE_SUCCESS,null,result);\n"
				+ "\t}";
		...
		wirteBaseDTO(packageLine, reqVOType, dir+reqVOType+".java",extendPageDTO);
	}
