...
public class MybatipseXmlUtil {
	private static final List<String> defaultTypeAliases = Arrays.asList("string", "map", "hashmap", "list", "arraylist", "collection", "iterator",
			"resultset",
	...
	public static String getNamespace(Document document) {
		try {
			...
			Log.error( "Invalid xpath (unexpected)", e);
		}
		return null;
	}
	...
	public static String getNamespaceFromActiveEditor(IJavaProject project) {
		...
		try {
			...
			Log.error("Failed to get raw classpath for project: " + project.getElementName(), e);
		}
		return null;
	}
	...
	public static Node findEnclosingStatementNode(Node parentNode) {
		try {
			return XpathUtil.xpathNode(parentNode,
					"ancestor-or-self::select|ancestor-or-self::update" + "|ancestor-or-self::insert|ancestor-or-self::delete");
		} catch (XPathExpressionException e) {
			Log.error( e.getMessage(), e);
		}
		return null;
