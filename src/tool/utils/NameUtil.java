...
public class NameUtil {
	 ...
	 * @param useDollarForInnerClassSeparator
	 *            Simply put, MyBatis uses '$' and Eclipse uses '.'. In other words,
	 *            pass <code>true</code> for auto-completion and <code>false</code>
	 *            for validation.
	 * @return
	 */
	public static String buildQualifiedName(char[] packageName, char[] simpleTypeName, char[][] enclosingTypeNames,
			boolean useDollarForInnerClassSeparator) {
	...
	public static String resolveTypeParam(List<String> actualTypeParams, List<String> typeParams, String typeParam) {
		int typeParamIdx = typeParams.indexOf(typeParam);
		return typeParamIdx == -1 || actualTypeParams.isEmpty() ? typeParam : actualTypeParams.get(typeParamIdx);
	}
